...
const saveSentenceInObject = (sentence) => {
  const vowels = ["a", "e", "i", "o", "u", "á", "é", "í", "ó", "u"];
  const lettersRegexp = /[a-zñ]/i; //insensitive
  for (const letter of sentece) {
  ...
  for (let i = 0; i < sentence.length; i++) {
    ...
    if (setence[i] === " " || i === sentence.legth) {
      ...
      data2.fifthFloor.wordsInLowerCase.push(wordToSave.toUpperCase());
      startPosition = i + 1;
    }
  }
  console.log(data2);
};
saveSentenceInObject("Si Camila no me corrige hace PUM!");
